		...
		t.Run(engine, func(t *testing.T) {
			...
			t.Run("旧配置升级并发触发及重启后次数仍受限", func(t *testing.T) {
				...
				for range 2 {
					...
					go func() {
						defer wg.Done()
						errors <- runChannelMonitorCustomAction(t.Context(), monitor, config.Actions[0], 5, "", time.Second, clock)
					}()
				}
				wg.Wait()
				...
				require.NoError(t, runChannelMonitorCustomAction(t.Context(), monitor, config.Actions[0], 5, "", time.Second, clock))
				...
				require.NoError(t, runChannelMonitorCustomAction(t.Context(), monitor, config.Actions[0], 20, "", time.Second, clock))
			...
			t.Run("失败不重试且满足冷却后恢复才可再次执行", func(t *testing.T) {
				...
				require.NoError(t, err)
				clock := func() time.Time { return now }
				err = runChannelMonitorCustomAction(t.Context(), monitor, action, 5, "", time.Second, clock)
				require.ErrorContains(t, err, "401")
				assert.NotContains(t, err.Error(), "secret-response")
				require.NoError(t, runChannelMonitorCustomAction(t.Context(), monitor, action, 5, "", time.Second, clock))
				require.NoError(t, db.Model(&model.ChannelRatioMonitor{}).Where("channel_id = ?", 2).Update("upstream_balance", 20).Error)
				require.NoError(t, runChannelMonitorCustomAction(t.Context(), monitor, action, 20, "", time.Second, clock))
				require.NoError(t, db.Model(&model.ChannelRatioMonitor{}).Where("channel_id = ?", 2).Update("upstream_balance", 5).Error)
				now = now.Add(59 * time.Minute)
				require.NoError(t, runChannelMonitorCustomAction(t.Context(), monitor, action, 5, "", time.Second, clock))
				assert.EqualValues(t, 1, calls.Load())
				now = now.Add(time.Minute)
				require.Error(t, runChannelMonitorCustomAction(t.Context(), monitor, action, 5, "", time.Second, clock))
				assert.EqualValues(t, 2, calls.Load())
			...
			t.Run("截止时间和旧结果不执行", func(t *testing.T) {
				...
				require.ErrorIs(t, runChannelMonitorCustomAction(t.Context(), monitor, action, 5, "", time.Second, clock), model.ErrChannelRatioMonitorConfigChanged)
			})
			...
			t.Run("凭据准备跨过截止时间不发起重置", func(t *testing.T) {
				...
				err = runChannelMonitorCustomAction(t.Context(), monitor, action, 5, "", time.Second, func() time.Time { return time.Unix(timestamp.Load(), 0) })
			...
			t.Run("倍率触发使用变量且成功判定失败不重试", func(t *testing.T) {
				...
				require.NoError(t, err)
				clock := func() time.Time { return now }
				err = runChannelMonitorCustomAction(t.Context(), monitor, action, 2, "", time.Second, clock)
				require.ErrorContains(t, err, "成功判定")
				require.NoError(t, runChannelMonitorCustomAction(t.Context(), monitor, action, 2, "", time.Second, clock))
				assert.EqualValues(t, 1, calls.Load())
			...
			t.Run("GET接口结果未知不被连接池自动重放", func(t *testing.T) {
				...
				now, err := time.Parse(time.RFC3339, "2026-09-13T04:00:00Z")
				require.NoError(t, err)
				err = runChannelMonitorCustomAction(t.Context(), monitor, action, 5, "", time.Second, func() time.Time { return now })
