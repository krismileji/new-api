...
func FilterUserTokenAutoGroups(userGroup string, groups []string) []string {
	return FilterRoleTokenAutoGroups(userGroup, common.RoleCommonUser, groups)
}
...
func GetRequestAutoGroups(c *gin.Context, userGroup string) []string {
	...
	if !ok {
		return GetRoleAutoGroups(userGroup, role)
	}
	groups, ok := value.([]string)
	if !ok {
		return []string{}
	}
	return FilterRoleTokenAutoGroups(userGroup, role, groups)
}
