...
const CHANNEL_MONITOR_AUDIT_TEMPLATES: Record<string, string> = {
  ...
  'channel.monitor_smart_schedule_channel_config_update':
    '已更新渠道 {{channel_label}} 的智能调度参与设置（影响 {{updated}} 条路由）',
  ...
  'channel.monitor_smart_schedule_route_exploration_clear':
    '已解除渠道 {{channel_label}} 在分组 {{group}}、模型 {{model}} 的临时探索状态，恢复优先级 {{priority}}、权重 {{weight}}',
  ...
  'channel.monitor_ratio_update':
    '已将渠道 {{channel_label}} 的倍率更新为 {{ratio}}',
  'channel.monitor_ratio_update_run': '已启动上游倍率更新任务 {{task_id}}',
  'channel.monitor_upstream_config_update':
    '已更新渠道 {{channel_label}} 的上游配置（{{upstream_type_label}}，成本换算：{{cost_conversion}}，换算系数 {{conversion_factor}}）',
  'channel.monitor_upstream_ratio_fetch':
    '已获取渠道 {{channel_label}} 的上游倍率 {{ratio}}，换算后成本倍率 {{cost_ratio}}（系数 {{conversion_factor}}）',
  'channel.monitor_upstream_balance_fetch':
    '已获取渠道 {{channel_label}} 的上游余额 {{balance}}',
  'channel.monitor_upstream_group_apply':
    '已将上游分组 {{group}} 应用于渠道 {{channel_label}}（已更新 {{keys_updated}} 个令牌，上游倍率 {{ratio}}，成本倍率 {{cost_ratio}}）',
  ...
  'channel.status_probe_config_update':
    '已更新渠道 {{channel_label}} 的状态探测配置（启用：{{enabled}}，间隔 {{interval_seconds}} 秒）',
  'channel.status_probe_config_changed':
    '已更新渠道 {{channel_label}} 的状态探测配置（{{status}}，{{model_count}} 个模型，间隔 {{interval_seconds}} 秒）',
  'channel.status_probe_run':
    '已请求立即探测渠道 {{channel_label}}（请求 {{manual_request_id}}）',
  ...
  'channel.model_detection_config_update':
    '已更新渠道 {{channel_label}} 的模型检测配置（定时检测：{{schedule_status}}，{{target_count}} 个目标）',
}
  ...
  if (fixedChineseTemplate) {
    ...
    if (params.channel_label == null) {
      ...
      if (channelName != null && channelId != null) {
        params.channel_label = `${String(channelName)}（ID: ${String(channelId)}）`
    ...
    const template =
      params.channel_name == null
        ? fixedChineseTemplate
        : fixedChineseTemplate.replaceAll(
            '{{channel_label}} ',
            '{{channel_label}}'
          )
