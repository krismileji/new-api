...
describe('usage log access tier', () => {
  ...
  test('keeps the sanitized aggregate view separate from privileged access', () => {
    assert.equal(
      resolveLogsViewAccess(ROLE.ADMIN, 'user-visible'),
      'user-visible'
    )
    assert.equal(
      resolveLogsViewAccess(ROLE.SUPER_ADMIN, 'user-visible'),
      'user-visible'
    )
  })
})
