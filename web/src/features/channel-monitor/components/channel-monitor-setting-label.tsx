...
const CHANNEL_MONITOR_SMART_SCHEDULE_SETTING_HELP = {
  ...
  stabilityRange: smartScheduleSettingHelp({
    ...
    range: '1–1440 的整数',
    defaultValue: '5',
    constraints: '不能超过实时样本保留时间，不替代请求级保护失败窗口',
  }),
  realtimeRetention: smartScheduleSettingHelp({
    meaning: '设定每个渠道模型在 Redis 中保留实时调度样本的时间。',
    unit: '分钟',
    range: '5–1440 的整数',
    defaultValue: '60',
    constraints: '不能短于性能窗口或稳定性评分窗口',
  }),
  realtimeSampleLimit: smartScheduleSettingHelp({
    meaning: '限制每个渠道模型在 Redis 中保留的实时调度样本数量。',
    unit: '条',
    range: '1000–200000 的整数',
    defaultValue: '20000',
    constraints: '时间和数量任一达到上限都会删除最早样本',
  }),
