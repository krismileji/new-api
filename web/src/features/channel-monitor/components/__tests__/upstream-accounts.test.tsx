...
describe('共享上游账户', () => {
  test('创建账户先预览多渠道差异，修改选择后必须重新预览', async () => {
    ...
    const search = screen.getByRole('searchbox', { name: '筛选关联渠道' })
    await user.type(search, '22')
    expect(
      screen.queryByRole('checkbox', { name: new RegExp(channel.name) })
    ).not.toBeInTheDocument()
    expect(screen.getByRole('checkbox', { name: /第二渠道/ })).toBeChecked()
    await user.clear(search)
    await user.type(search, '没有这个渠道')
    expect(screen.getByText('没有匹配的渠道，请调整筛选条件。')).toBeVisible()
    await user.clear(search)
    await user.click(screen.getByRole('button', { name: '仅看已选（2）' }))
    expect(
      screen.getByRole('button', { name: '仅看已选（2）' })
