...
describe('channel monitor cost history dialog layout', () => {
  ...
  test('shows channel remarks, status, and cost ratios in the channel view', () => {
    ...
    assert.equal(markup.includes('min-w-[1080px]'), false)
    assert.ok(markup.includes('w-max min-w-full table-auto'))
    assert.match(markup, /w-max px-1 text-xs/)
    assert.ok(markup.includes('[&amp;_td]:overflow-hidden'))
    assert.ok(markup.includes('[&amp;_td]:text-ellipsis'))
    assert.ok(markup.includes('grid-cols-3'))
