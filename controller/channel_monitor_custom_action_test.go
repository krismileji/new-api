...
func TestChannelMonitorCustomActionRefreshIntegration(t *testing.T) {
	...
	outcome, err := fetchAndRecordChannelMonitorUpstreamRatio(t.Context(), monitor, nil, "", time.Second, true, 0, "测试")
	require.NoError(t, err, "重置失败不能使指标获取失败或触发获取重试")
	assert.Equal(t, 2.0, outcome.Result.Ratio)
	assert.EqualValues(t, 1, calls.Load())
	_, _, err = fetchAndRecordChannelMonitorUpstreamBalance(t.Context(), monitor, nil, "", time.Second)
	require.NoError(t, err)
