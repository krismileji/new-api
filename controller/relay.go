...
func executeTaskSubmissionWith(
	...
	if err != nil {
		...
		diagnostics.failed("retry_snapshot", "snapshot_error", taskErr, false)
		return nil, taskErr
	}
	...
	for retryParam.GetRetry() <= common.RetryTimes {
		...
		if requestErr := c.Request.Context().Err(); requestErr != nil {
			diagnostics.cancelled("before_attempt", attemptIndex+1)
		...
		diagnostics.attempt(attemptIndex+1, channel, relayInfo.LockedChannel != nil)
		...
		if requestErr := c.Request.Context().Err(); requestErr != nil {
			diagnostics.cancelled("after_submit", attemptIndex+1)
			taskErr = service.TaskErrorWrapperLocal(requestErr, "request_cancelled", http.StatusRequestTimeout)
		}
		if taskErr == nil {
			successfulChannelID = channel.Id
			diagnostics.attemptSucceeded(attemptIndex+1, result)
			break
		}
		service.FinalizeChannelDailyCostAttempt(c, channel.Id, false)
		...
		diagnostics.attemptFailed(attemptIndex+1, channel, taskErr, shouldRetry)
		if !shouldRetry {
			break
		}
		if retryDecision == relayRetryFastFailureSameChannel &&
