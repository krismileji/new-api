...
var channelMonitorAuditContentTemplates = map[string]string{
	...
	"channel.monitor_concurrency_limit_update":               "已将渠道 ${id} 的并发限制更新为 ${concurrency_limit}（0 表示不限制）",
	"channel.monitor_smart_schedule_config_update":           "已将渠道 ${id} 在分组 ${group}、模型 ${model} 的主渠道固定时间更新为 ${duration_minutes} 分钟",
	"channel.monitor_smart_schedule_stability_clear":         "已手动解除渠道 ${id} 的稳定性保护，恢复优先级 ${priority}、权重 ${weight}",
	"channel.monitor_smart_schedule_channel_config_update":   "已更新渠道 ${id} 的智能调度参与设置（影响 ${updated} 条路由）",
	"channel.monitor_smart_schedule_route_config_update":     "已更新渠道 ${id} 在分组 ${group}、模型 ${model} 的智能调度参与设置",
	"channel.monitor_smart_schedule_group_pause_update":      "已将渠道 ${id} 在分组 ${group}、模型 ${model} 的流量暂停时间更新为 ${duration_minutes} 分钟",
	"channel.monitor_smart_schedule_route_stability_clear":   "已解除渠道 ${id} 在分组 ${group}、模型 ${model} 的稳定性保护，恢复优先级 ${priority}、权重 ${weight}",
	"channel.monitor_smart_schedule_route_exploration_clear": "已解除渠道 ${id} 在分组 ${group}、模型 ${model} 的临时探索状态，恢复优先级 ${priority}、权重 ${weight}",
	...
	"channel.status_probe_config_update":                     "已更新渠道 ${channel_id} 的状态探测配置（启用：${enabled}，间隔 ${interval_seconds} 秒）",
	"channel.status_probe_config_changed":                    "已更新渠道 ${channel_id} 的状态探测配置（${status}，${model_count} 个模型，间隔 ${interval_seconds} 秒）",
	"channel.status_probe_run":                               "已请求立即探测渠道 ${channel_id}（请求 ${manual_request_id}）",
	"channel.model_detection_settings_update":                "已更新模型检测设置（定时检测：${schedule_status}，预设：${scheduled_preset}，间隔 ${interval_minutes} 分钟）",
	"channel.model_detection_config_update":                  "已更新渠道 ${channel_id} 的模型检测配置（定时检测：${schedule_status}，${target_count} 个目标）",
}
