...
倍率更新任务可设置 `0` 到 `525600` 分钟的间隔，`0` 表示关闭。每个失败渠道默认最多重试 `3` 次，可配置 `0` 到 `10` 次；每次重试前默认等待 `0` 秒（可配置 `0` 到 `600` 秒），`0` 表示立即重试。Sub2API 认证失败不会做无意义的重复认证请求。倍率或余额分别连续失败默认达到 `10` 次后停止自动更新，可在设置中调整停止次数。
