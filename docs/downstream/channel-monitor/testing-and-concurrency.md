...
固定主渠道的稳定性回归需要分别覆盖两种模式。省略 `allow_stability_degrade` 或显式设为 `true` 时，连续失败或保护失败窗口内累计失败达到策略阈值应立即降级路由，但不清除固定到期时间和选主意图；分钟级稳定性评分低于兼容降级字段不能单独触发硬保护。在到期前完成试放并达到恢复条件后，该路由应重新成为固定主渠道。显式设为 `false` 时，同样的异常样本应继续进入评分和记录，但固定期内不应自动降级。
