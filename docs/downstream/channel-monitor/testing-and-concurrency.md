...
全局智能调度已启用且能确定测试的原始模型时，所有已配置、未禁用并正在参与调度的对应渠道、分组、模型路由都会收到本次手动连接测试样本。记录不受“关闭”、“探索流量”或“定时探测”选择限制，稳定性降级中的参与路由也会记录；一次测试可同步覆盖多个匹配的参与分组。流式成功样本在可用时记录首字时间和 TPS，非流式成功样本只记录成功，失败样本记录错误和本次测试耗时。测试仍只保留一条模型测试消费日志和一份渠道成本，模型测试日志不计入普通业务分钟统计，避免重复计算。
...
固定主渠道的稳定性回归需要分别覆盖两种模式。省略 `allow_stability_degrade` 或显式设为 `true` 时，稳定性低于阈值应临时降级路由，但不清除固定到期时间和选主意图；在到期前完成试放并达到恢复阈值后，该路由应重新成为固定主渠道。显式设为 `false` 时，同样的异常样本应继续进入分数和记录，但固定期内不应自动降级。
