...
启用智能调度且对应分组选择“定时探测”时，支持文本探测且真实发往上游的手动连接测试会同时补充该渠道和模型的探测样本；一次测试可同步覆盖多个符合条件的参与分组。测试仍只保留一条模型测试消费日志和一份渠道成本，模型测试日志不计入普通业务分钟统计，避免重复计算。请求未发往上游的本地校验或转换失败不会补样本。
