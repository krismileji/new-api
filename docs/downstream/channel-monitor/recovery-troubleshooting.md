...
少量正在处理的新事件可以与正常状态同时出现。历史累计计数不会在恢复后自动清零；日常查看“今日诊断”即可，也可以用“重置今日计数”从当前时间重新统计。

## 当前状态与累计诊断

运行详情默认展示“今日诊断”，将当前恢复、异常原因、待处理和清理状态放在“后台恢复”中；过往隔离记录和共享累计计数放在默认收起的“历史诊断（累计）”中。展开历史不会清除记录，收起历史也不会屏蔽新增隔离或当前清理故障。正常状态中的“后台待处理”可能包含正在流转的新事件，不能仅凭数量大于零认定恢复失败。

今日诊断按北京时间的自然日统计后台重试、接管、隔离和两种清理失败次数，使用 Redis 时间统一多节点的日期。换日后重新计数，页面沿用现有的手动刷新规则，并显示统计起点和查询时间。首次启用时从实际起点记录，不能从没有日期信息的历史累计数反推之前的今日计数；页面会明确提示这段统计范围。

“重置今日计数”经确认后在服务端原子归零这五项当日计数，后续事件继续累加；所有管理端共享重置结果。接口校验用户确认的统计日期，跨午夜的旧确认不能清掉新一天的计数。重置不会改变当前故障标记、待处理任务、隔离明细、历史累计或通知检测，因此不会把正在发生的故障改成正常。

| 指标 | 实际含义 |
| --- | --- |
| 事件处理重试（累计） | 包含事件处理失败后的重试与消费者自动恢复；同一事件可多次计数，也可能按失败批次中的事件数量累加 |
| 自动接管（累计） | 重新领取闲置待处理事件的数量，同一事件可能被多次接管；不等于服务宕机次数 |
| 异常隔离（累计） | 曾进入隔离队列的事件总数；需要结合隔离原因判断影响，不等于请求失败数或统计丢失数 |
| 标记、事件清理失败（累计） | 过往清理失败次数；当前是否仍失败由各自的清理状态表示 |

以上历史累计保存在共享 Redis 的 `channel_monitor:v1:observability` 中，没有每日归零或随恢复、应用重启清零的逻辑。后续发生相应事件会继续累加，没有新增重试、接管、隔离或清理失败时保持不变。它们是少量数值字段，数字增长不等于存储了同等数量的明细。

“数据保留”设置中的数据库清理与诊断计数的保留范围不同：

| 数据 | 保留与清理方式 |
| --- | --- |
| 今日诊断 | 只使用 `channel_monitor:v1:diagnostics:today` 一份固定大小的 Hash，换日覆盖；停止使用后最迟在所属日结束后的 24 小时过期，不会每天增加一个新键 |
| 历史累计 | 固定的数值字段，供历史排查及新增故障检测使用；不跟随数据库清理或今日重置归零 |
| 隔离明细 | Redis 队列按约 10,000 条进行长度裁剪，当前不按数据库保留天数清理；裁剪不会减少历史累计 |
| 数据库中的成本、任务、执行明细等历史 | 继续由已有“数据保留”选项及自动清理任务控制，本次没有改动这些删除规则 |

一份页面快照只能说明该次观测时的状态，无法排除间歇复发。检查下文的计数增量及最近隔离日志，确认隔离数量和时间是否继续变化；不要通过清空计数来证明问题已经解决。

本次今日诊断改动未修改数据库模型、SQL、迁移或数据库清理任务。使用 Redis 8.8.0 的专用临时实例验证当日计数、重置后继续计数、故障标记及 pending/隔离记录保留；确定性测试另覆盖北京时间跨日、拒绝过期日期的重置、首次启用不回填历史累计、无逐日键积累及过期回收。页面回归覆盖确认与取消、重复提交保护、失败保留计数和读取失败时禁用重置。

```powershell
$env:TEST_MONITOR_DIAGNOSTICS_REDIS_ADDR='127.0.0.1:26390'
& 'D:\Go\sdk\go1.26.5\bin\go.exe' test ./service -run '^TestChannelMonitor(Diagnostics|Redis|Recovery)' -count=1
& 'D:\Go\sdk\go1.26.5\bin\go.exe' test ./controller -run '^Test(ChannelMonitorDiagnostics|GetChannelMonitorRecoveryBeforeBackgroundCheck)' -count=1
```
