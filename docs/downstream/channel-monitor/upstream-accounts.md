...
账户刷新间隔为 0 时关闭定时余额刷新，手动查询和自动任务仍可按需读取。余额恢复是否自动启用遵循现有「余额恢复后自动启用」设置及倍率／健康检查条件；手动停用的渠道保持停用。Redis 未启用时保存上游原始余额，实时费用预估不可用；常规监控不依据不完整估算恢复渠道。独立自动任务按成功复查的上游原始余额判断恢复，不等待本地预估完整。
