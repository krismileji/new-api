...
- 余额任务按账户执行；倍率规则必须选择渠道。迁移由用户选择配置来源与关联渠道，展示差异并保留任务历史和限制。
...
4. 点击「合并关联任务」，选择保留的目标任务并预览规则。倍率规则还须选择账户内的来源渠道。确认后目标任务保留调度设置，相同规则合并计数，其余任务停用并保留历史。未确认的执行仍须人工核对，不能借合并绕过。
...
关联操作不会按域名自动推断账户，也不会自动合并任务。关联后的旧渠道任务在合并前停止执行；新建账户任务时若仍有旧关联任务，必须先合并，避免丢失旧调用次数。解除关联后渠道保留当前配置独立运行，在途请求继续结算到原余额池。删除账户须先解除成员并删除或处理引用它的自动任务。

每个账户最多关联 100 个渠道，一次最多合并 20 个任务。Sub2API API Key 账户创建时需使用单 Key 来源渠道，账户单独保留余额查询 Key；各渠道的转发 Key 和倍率查询仍保持各自归属。空账户可重新关联渠道或删除；共享配置编辑使用其关联渠道作为操作入口。
...
在「上游自动任务」选择「自定义（可单独关联账户余额）」，同样可在「上游余额来源」选择账户。任务的地址、代理、倍率、自定义变量和触发请求保持独立，动作前后读取账户余额；账户凭据不会注入任务动作。此类任务保留自身规则和调用限制，不参加整套账户任务的合并。「测试获取指标」只读取指标，不执行动作、不改写账户快照或渠道状态。
...
7 个前端测试文件、44 个用例通过；类型检查、涉及文件 lint、生产构建通过。新增界面采用延迟加载，保留全部现有版权头，未新增翻译键或修改语言包。后端新增回归覆盖账户轮询去重、不同渠道倍率保留、旧结果拒绝、成员保护、手动停用保留、凭据刷新、合并调用限制、内置账户认证继承及任务只执行一次。
