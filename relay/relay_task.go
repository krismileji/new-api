...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	//    ResolveOriginTask 可能已在 remix 路径中预设了 OtherRatios，同名值以原任务为准。
	...
	if resp.Body == nil {
		...
		responseErr := relaykittypes.NewErrorWithStatusCode(
			errors.New("upstream response body is nil"),
			relaykittypes.ErrorCodeBadResponseBody,
			statusCode,
		)
		if statusCode >= http.StatusOK && statusCode < http.StatusMultipleChoices {
			relaykittypes.ErrOptionWithSkipRetry()(responseErr)
		}
	...
	if taskErr != nil {
		...
		taskErr.Error = relaykittypes.NewErrorWithStatusCode(
			responseErr,
			relaykittypes.ErrorCodeBadResponseBody,
			taskErr.StatusCode,
			relaykittypes.ErrOptionWithSkipRetry(),
		)
		return nil, taskErr
	}
	if parsed == nil {
